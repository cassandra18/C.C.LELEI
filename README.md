# Hey, I'm Cassandra. 👋

I'm an electrical engineer who writes code and thinks in systems.  
Currently finishing my degree, running two startups under **LeleiMade Co. Ltd**, and looking for the kind of work that actually matters.

---

### What I do

- ⚡ **Electrical Engineering** — power systems, circuit design, renewable energy
- 💻 **Software Development** — Python, JavaScript, APIs, web applications
- 🧠 **AI & Automation** — applied machine learning, prompt engineering, intelligent systems

---

### What I'm building

| Project | What it is |
|---|---|
| 🏘️ Real Estate Management Software | Property management platform built for the African market |
| ⚡ Virtual Power Plant as a Service | Distributed energy intelligence for communities and businesses |

Both under **[LeleiMade Co. Ltd](https://github.com/cassandra18/LELEIMADE.CO-PROPERTY-MANAGEMENT-SAAS)** — my attempt to build infrastructure that outlasts me.

---

### Background

ALX Software Engineering · ALX AI Career Essentials · freeCodeCamp Responsive Web Design  
Electrical Engineering degree · Graduating June 2025

I'm Kalenjin. My people read the land, the sky, and the seasons.  
I bring that same attentiveness to every system I build.

---

### Right now

- 🔭 Working on: VPP-as-a-Service architecture + real estate platform MVP
- 🌱 Deepening: system design, cloud infrastructure, energy data modeling
- 📬 Open to: engineering roles, technical collaborations, interesting problems

---

### Find me

[![Portfolio](https://img.shields.io/badge/Portfolio-cassandra18.github.io-C0152B?style=flat-square)](https://github.com/cassandra18/C.C.LELEI)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-1A2E5A?style=flat-square&logo=linkedin)](https://linkedin.com/in/cassandra-lelei)
[![Email](https://img.shields.io/badge/Email-Say%20hello-E8621A?style=flat-square)](mailto:leleimade@gmail.com)

---

*"The best engineering listens before it speaks."*
